...
<body>
  <div class="home-container">
    <div class="top-nav">急上昇</div>
    <div class="top-nav-icons">☀ ⚙</div>
    
    <div class="brand-title">INVIDIOUS</div>
...
// 2. 検索結果画面のHTML生成
const renderSearchResults = (query, videos) => {
  const videoCards = videos.map(video => {
    // youtube-search-api のレスポンスから必要なデータを抽出
    ...
    const viewCount = video.viewCount?.short || video.viewCount?.text || '';
    const published = video.publishedTimeText || '';

    return `
      <div class="video-card">
        <a href="/watch?v=${videoId}" class="thumbnail-wrapper">
          <img src="${thumbnail}" alt="${title}" loading="lazy">
          ${duration ? `<span class="duration">${duration}</span>` : ''}
        </a>
        <div class="video-info">
          <h3 class="video-title"><a href="/watch?v=${videoId}">${title}</a></h3>
          <div class="channel-name">${channelName} <span class="verified">✔</span></div>
...
<body>
  <div class="navbar">
    <div class="nav-left">
      ...
      <form class="search-form nav-search" action="/search" method="GET">
...
app.get('/search', async (req, res) => {
  ...
  try {
    // youtube-search-api を使用して検索 (APIキー不要)
    const searchData = await yts.GetListByKeyword(query, false, 24);
    
    // searchData.items に動画情報が配列で格納されている
    const videos = searchData.items.filter(item => item.type === 'video');
    
    // HTMLテンプレートに流し込んでレスポンス
    res.send(renderSearchResults(query, videos));

  } catch (error) {
    console.error('Search API Error:', error);
    res.status(500).send(`
      <body style="background:#1a1a1a;color:#fff;padding:50px;font-family:sans-serif;">
        <h2>エラーが発生しました</h2>
        <p>${error.message}</p>
        <a href="/" style="color:#aaa;">ホームに戻る</a>
      </body>
    `);
  }
});
...
app.get('/watch', (req, res) => {
  ...
  res.send(`
    <body style="background:#1a1a1a;color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;">
      <h2>動画再生ページ (ID: ${videoId})</h2>
      <p style="color:#aaa;margin-top:10px;">ここに独自のプレイヤーやスクレイピング処理(scrapeYouTubeMeta等)を実装します。</p>
      <a href="javascript:history.back()" style="color:#fff;margin-top:20px;text-decoration:none;border:1px solid #555;padding:10px 20px;">戻る</a>
    </body>
  `);
});
